...
class Black_Litterman_Portfolio():
    ...
    def __init__(self, data_reader_config, risk_aversion, residual_diagonalized=True, factor_diagonalized=True):
        ...
        self._residual_diagonalized = residual_diagonalized
    ...
    def single_period(self, target_date, verbosity=False):
        ...
        if self._residual_diagonalized:
            D = np.diag(RESIDUAL_COV.ix[TRADING_UNIVERSE][TRADING_UNIVERSE].as_matrix().diagonal())
        else:
...
def main():
    ...
    DR_residual = BLDR.residual_data_reader("Data/residual.csv",
                                        look_back_periods=60, threshold = None, min_periods=2, diagonalized=False)
    ...
    BLP = Black_Litterman_Portfolio(data_reader_config, 1, residual_diagonalized=True, factor_diagonalized=False)
